...
public class EchoServer {
    ...
    private static void handleClient(Socket socket) throws IOException {
        ...
        Logger.getLogger(Log.LOG_NAME).log(Level.INFO, "Received the message: " + message);
        //System.out.println(String.format("Received the message: %1$S ", message));
        while (!message.equals(ProtocolStrings.STOP)) {
            writer.println(message.toUpperCase());
            Logger.getLogger(Log.LOG_NAME).log(Level.INFO, "Received the message: " + message.toUpperCase());
